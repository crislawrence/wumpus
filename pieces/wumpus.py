...
class Wumpus(Hazard):
    ...
    def establish_hazard_perimeter(self):
        ...
        self.hazard_perimeter = Hazard_Perimeter(
            [StatusMessage('WARNING', 'WUMPUS', "You smell a wumpus (ick!)")],
            set(surrounding_cave_ids))

    def awakened(self):
        self.asleep = False
        return [StatusMessage('INFO', 'WUMPUS', "Wumpus is awake and stiring!")]

    def move(self, hunter, hazards=None):
        status = []
        if not self.asleep:
            destination = random.choice(self.cave.neighboring_caves)
            self.cave = self.cavern_system.get_cave(destination)
            status.extend(self.check_encounter(hunter, hazards))
        return status
    ...
    def react_to_shot(self, cave_id, hunter, hazards=None):
        status = []
        if cave_id == self.cave.id:
            status.extend(self.killed())
        elif self.asleep:
            status.extend(self.awakened())
            status.extend(self.move(hunter, hazards))
        else:
           status.extend(self.move(hunter, hazards))
        return status
    ...
    def killed(self):
        self.alive = False
        return [StatusMessage('TERMINAL', 'WUMPUS', "You have slain the wumpus!")]
